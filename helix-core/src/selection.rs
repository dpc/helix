...
impl Selection {
    ...
    pub fn ensure_invariants(self, text: RopeSlice) -> Self {
        self.transform(|r| r.grapheme_aligned(text))
            .normalize()
    }
...
mod test {
    ...
    fn test_backward_secondary_selection_preserved() {
        ...
        let selection = Selection::new(
            smallvec![Range::new(0, 5), Range::new(11, 6)],
            0,
        );
        ...
        assert!(secondary.head < secondary.anchor, "Secondary should be backward");
        ...
        assert!(backward_range.is_some(), "Should still have range covering 6-11");
    ...
    #[test]
    fn test_backward_selection_merge_behavior() {
        // Verify behavior when merging backward selection with point
        let text = Rope::from_str("Hello world!");

        // Create a backward selection
        let backward = Range::new(5, 0); // anchor=5, head=0 (backward)
        assert_eq!(backward.direction(), Direction::Backward);

        // Merge with a point at the same position
        let point = Range::new(0, 0);
        let merged = backward.merge(point);

        // Check what happens - the merge function behavior
        // Currently: if not both backward, result is forward
        println!(
            "Merged backward({}, {}) with point({}, {}): result is ({}, {})",
            backward.anchor, backward.head, point.anchor, point.head, merged.anchor, merged.head
        );

        // The current behavior: merging backward + point = forward
        // This might be the bug! Let's document what we see.
